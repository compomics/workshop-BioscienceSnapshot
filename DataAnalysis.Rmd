...
We will work with the same dataset as in the first part of the workshop. In the paper, the authors found two distinct groups of humans based on the bacterial species they found in the samples. Here we will now try to find proteins that are differentially expressed between the two groups.
...
FlashLFQ has a GUI, can be run from the command line and has a docker image. You can run FlashLFQ command line from within an RMarkDown file by adding a bash chunk. To run it on Linux, you need .NET core. To see all the arguments, run the help parameter.

```{bash}
dotnet C:/Users/Nina/Documents/FlashLFQ1.2.3/CMD.dll --help
```

Let's keep it simple for now and try to quantify 2 files from the project. You can find the files here:
...
"""
It is always difficult to know what the "right" arguments to the parameters are (for any quantification algorithm).
It is often necessary to know a lot about the dataset and also just evaluate many options, which is not always doable.

here just one or two files, see how fast it is
"""

```{bash eval=FALSE}
dotnet /home/compomics/Projects/FlashLFQ_1.2.2/CMD.dll --idt "/home/compomics/Projects/PS_FlashLFQ_pipeline/output/entire_project/input_FlashLFQ.txt" --rep "/public/compomics/Nina/E.Coli_dataset_PXD003881/mzml/" --out "/home/compomics/Projects/PS_FlashLFQ_pipeline/output/entire_project/" --ppm 5 --mbr --ath
``` 
...
We have already done the quantification for the entire project for you. Read this QuantifiedPeptides file into R environment.

```{r}
QP <- read.csv2("F:/Doctoraat/copenhagen_teeth/ionbot_out/QuantifiedPeptides.tsv", sep="\t")
```


# Lowest Common Ancestor calculation with UniPept (optional)

https://unipept.ugent.be/
https://doi.org/10.1021/acs.jproteome.8b00716

And the end of the data analysis pipeline we want to assess from which bacterial species the differential proteins (if any) originate. 
...
You can find the small set of peptides in the following file: 
...
knitr::include_graphics("C:/Users/Nina/OneDrive - UGent/Documenten/Doctoraat/Bioscience snapshot Copenhagen/QFeaturesRepresentatie.png")
```
...
```{r}
peptidesFile <- "C:/Users/Nina/OneDrive - UGent/Documenten/Doctoraat/Bioscience snapshot Copenhagen/archeologische_tanden_dataset/QuantifiedPeptidesUniPept.tsv"
```
...
We have seen in the paper that two groups were identified between the samples based on the presence of certain bacterial species. We can add this information into the colData.
In the following analysis, we will investigate whether there are any proteins differentially expressed between these two groups and we will check to which bacterial species they belong.


We update the colData with information on the design

```{r}
colData(pe)$group <- c(rep("group1", 3), rep("group2", 4))
```
...
Iets van dat het al is uitgefilterd of zo?
...
```{r}
contaminants <- read.delim("C:/Users/Nina/OneDrive - UGent/Documenten/Doctoraat/Bioscience snapshot Copenhagen/archeologische_tanden_dataset/contaminants.txt", sep = "\t")
```
...
2. Drop peptides that were only identified in one sample
...
Visualise most common bacterial species per group? Or try to see a split between the groups

```{r eval=FALSE, include=FALSE}
QP %>% filter(genus != " ") %>% pivot_longer() %>% ggplot(aes())
```
...
Can you visualise the effect of the normalisation? Why is it necessary?

```{r fig1, fig.height = 10, fig.width = 7}
...
how do you do that? by modeling the intensities of sample i en peptide p with sample specific effect and peptide specific effect, than you will have sample average (beta samp) that is corrected for peptide species (addition of betap)
...
rowData(pe[["peptideNorm"]])$Protein.Groups <- sapply(rowData(pe[["peptideNorm"]])$Protein.Groups, function(x){
  ifelse(x[1]==";", substr(x, 2, nchar(x)), x)
}, USE.NAMES = F)
```
...
In this example, this is of course very simple because we have only one model parameter "group".
Group 1 is the reference class. So the mean log2 expression for samples from group1 is '(Intercept)'.
The mean log2 expression for samples from group2 is '(Intercept)+groupgroup2'.
Hence, the average log2 fold change between group2 and
group1 is modelled using the parameter 'groupgroup2'.
Thus, we assess the contrast 'groupgroup2 = 0' with our statistical test.


```{r}
L <- makeContrast("groupgroup2=0", parameterNames = c("groupgroup2"))
...
```{r,warning=FALSE}
volcano <- ggplot(rowData(pe[["protein"]])$groupgroup2,
...
```{r}
sigNames <- rowData(pe[["protein"]])$groupgroup2 %>%
...
```{r}
rowData(pe[["protein"]])$groupgroup2 %>%
  filter(adjPval<0.05)
```
