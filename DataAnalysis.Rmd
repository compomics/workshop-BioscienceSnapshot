...
```{bash}
dotnet C:/Users/Nina/Documents/FlashLFQ1.2.3/CMD.dll --idt "http://genesis.ugent.be/uvpublicdata/workshop-bioscience-snapshot/id_files/flashlfq_input.tsv" --rep "http://genesis.ugent.be/uvpublicdata/workshop-bioscience-snapshot/RAW/" --out "~/FlashLFQ_out/" --ppm 5 --mbr --ath
``` 
...
And the end of the data analysis pipeline you might want to assess from which bacterial species the differential proteins (if any) originate. 
...
We have seen in the paper that they used both archeological and modern samples. We can add this information into the colData based on the colnames of the data object.
In the following analysis, we will investigate whether there are any proteins differentially expressed between these groups and we will check to which bacterial species they belong.
...
rowData(pe[["peptideNorm"]])$Protein.Groups <- sapply(rowData(pe[["peptideNorm"]])$Protein.Groups, function(x){
  ifelse(substr(x, 1, 1)==";", substr(x, 2, nchar(x)), x)
}, USE.NAMES = F)
```
